...
export default function SportNutrition() {
  ...
  useEffect(() => {
    ...
    if (categoryParam) {
      setCategories({
        протеїн: categoryParam === "протеїн",
        креатини: categoryParam === "креатини",
        аміно: categoryParam === "аміно",
        вітаміни: categoryParam === "вітаміни",
      });
    }
    ...
    const fetchProducts = async () => {
      try {
        const response = await fetch(
          "https://localhost:7164/api/products/sport-nutrition"
        );
  ...
  const handleCategoryChange = (e) => {
    ...
    const newCategories = { ...categories, [value]: checked };
    setCategories(newCategories);
    setCurrentPage(1);

    const activeCategories = Object.entries(newCategories)
      .filter(([_, isActive]) => isActive)
      .map(([category]) => category);

    if (activeCategories.length === 1) {
      navigate(`/sport-nutrition?category=${activeCategories[0]}`);
    } else {
      navigate("/sport-nutrition");
    }
  };
  ...
  const filteredProducts = products.filter((p) => {
    ...
    const okCat = categories[p.category];
    ...
    <div className="page">
      ...
      <main className="product-page">
        <div className="container product-page__container">
          <h1 className="product-page__title">Спортивне харчування</h1>
